...
- notifier is called with proper arguments, but we can omit it due to the fact that this will be tested in the integration test like: `test_get_gata_and_notify` and we want to unit test whole flow not side effects

> Make another test for the notifier if you want to test it separately with e.g. `mocks`.
